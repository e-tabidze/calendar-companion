  ...
  return (
    <Menu as='div' className='relative text-left hidden md:flex'>
      {({ open }) => (
        <>
          <Menu.Button>
            <AvatarContainer className={`hidden lg:flex ${open ? 'bg-grey-100 border-raisin-30' : ''}`}>
              ...
              <AvatarResponsiveContainer>
                ...
                >
                  {!!activeCompany
                    ...
                    : userInfo?.information?.first_name || userInfo?.FirstName}
                </Typography>
                <Icon
                  svgPath='chevron'
                  width={8}
                  height={6}
                  className={`${open ? 'rotate-180' : ''} fill-transparent flex ml-2 transition-all`}
                />
              </AvatarResponsiveContainer>
            </AvatarContainer>
                        ...
                        <div className='cursor-pointer px-4 py-3 hover:bg-grey-100 flex items-center justify-between'>
                          <div className='flex items-center text-2sm'>
                            ...
                            <div className='flex flex-col'>
                              <span className='text-2sm overflow-hidden text-ellipsis whitespace-nowrap max-w-[140px] inline-block'>
                                {userInfo?.information?.first_name || userInfo?.FirstName}{' '}
                                {userInfo?.information?.last_name || userInfo?.LastName}
                              </span>
                            ...
                            <li key={route.id}>
                              {route.path ? (
                                ...
                                <div className='border-t-1 border-raisin-10 mt-2 py-2'>
                                  <button className={`w-full ${routeClass}`} onClick={handleLogout} type="button">
                                    {t(route.item)}
                                  </button>
                                </div>
                              )}
