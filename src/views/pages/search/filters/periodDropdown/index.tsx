...
const PeriodDropwodn: React.FC<Props> = ({ control, resetField, setValue }) => {
  ...
  return (
    <Menu as='div' className='flex text-left w-full h-full'>
      {({ open }) => (
        <>
          <Menu.Button className='h-full bg-transparent px-3 lg:px-4 inline-flex justify-center items-center rounded-md text-sm font-medium text-white'>
            <FilterContainer>
              <InnerFilterContainer>
                ...
                {startDate || endDate ? (
                  ...
                  <Icon
                    ...
                    width={8}
                    height={6}
                    className={`${open ? 'rotate-180' : ''} transition-all fill-transparent ml-2`}
                  />
                )}
              </InnerFilterContainer>
            </FilterContainer>
