    ...
    useEffect(() => {
        if (!isFirstRender.current) {
            ...
            if (updateOrderRef.current) {
                setValue('order_by', selectedFilter?.order_by || ''); // Use optional chaining to avoid errors
                onClick();
                ...
                render={({ field: { onChange, value } }) => (
                    ...
                    >
                        <div className='relative flex text-left w-full ml-2'>
                            <Listbox.Button
                                className='relative flex items-center w-full cursor-pointer rounded-xl bg-white py-2 px-4 text-left border border-raisin-10 text-2sm'>
                                <Icon svgPath='sort' width={20} height={12}
                                      className='fill-transparent flex shrink-0 mr-3'/>
                                <Typography type='subtitle'
                                            className='text-2sm max-w-[140px] inline-block overflow-hidden text-ellipsis whitespace-nowrap'>
                                    {sortFilters.find(opt => opt.id === value)?.label || 'დაჯგუფება'}
                                </Typography>
                                <Icon svgPath='chevron' width={8} height={6}
                            ...
                            >
                                <Listbox.Options
                                    className='absolute top-full mt-2 z-10 w-full origin-top-right divide-y divide-gray-100 rounded-2xl bg-white shadow-lg focus:outline-none'>
