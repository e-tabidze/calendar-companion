      ...
      <>
        <DefaultLayout fullWidth={true}>
          ...
          <FullContainer className="laptop:flex">
            <SearchContentsContainer className={`w-full px-5 large:pl-10 transition-all duration-300 ${mapVisible ? 'laptop:w-1/2 pr-8': 'w-full laptop:pr-0'}`}>
                <SearchResultsContainer>
                  <Typography type='h5' weight='normal' className='mr-2 mb-5 large:mb-0'>
                    სულ ნაპოვნია 71 განცხადება
                  </Typography>
                  <div className='flex items-center my-4'>
                    <span className={`cursor-pointer ml-3 hidden laptop:flex items-center justify-center w-8 h-8 rounded-full ${mapVisible ? '':'bg-green-10'}`}>
                ...
                <div className={`grid tablet:grid-cols-2 gap-6 ${mapVisible ? 'grid-cols-2 2xl:grid-cols-3' : 'laptop:grid-cols-4 2xl:grid-cols-5'}`}>
                  <ProductCard />
                  <ProductCard />
                  <ProductCard />
                  <ProductCard />
            ...
            {/*{mapLayout && width > 779 && (*/}
            <MapContainer className={`absolute z-[111] top-[197px] laptop:top-[0] w-full left-0 laptop:relative overflow-hidden transition-all duration-300 ${mapVisible ? 'h-[100vh] laptop:w-1/2': 'h-0 laptop:h-[100vh] laptop:w-[40px]'}`}>
              <MapPicker width="100%" height="100vh"
                      className={`${mapVisible ? '' : 'after:content-[\'\'] after:bg-white after:absolute after:z-[11] after:top-0 after:left-0 after:w-full after:h-full'}`}
              />
